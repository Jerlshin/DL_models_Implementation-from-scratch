...
""" PUT TOGETHER 
COMBINED General things like...IoU, NMS, mAP, and functions for convertion 
...
def mean_average_precision(
    ...
    for c in range(num_classes):
        ...
        # amount of bboxes for each training example
...
def get_evaluation_bboxes(loader, model, iou_threshold,
                          anchors, threshold, box_format="midpoint", 
                          device="cuda",
    ):
...
# converting the pred from model to be relative to the entire image
def cells_to_bboxes(predictions, anchors, S, is_preds=True): # anchors used in prediction
    # prediction = [N, 3, S, S, num_classes + 5]
    # is_preds = is the input is prediction or the true bbox
...
def get_mean_std(loader):
    # var[X] = E[X**2] - E[X]**2 - prob and statistics class - variance formula 
...
def load_checkpoint(checkpoint_file, model, optimizer, lr):
    ...
    # If we don't do this then it will just have learning rate of old checkpoint and it will take many hours of debugging 
...
# seeding all the neccessay stuffs
